    ...
    var body: some View {
        List {
            if session.isMerchant {
                Section("Giocatori collegati \(session.state.players.count)/7") {
                    ...
                    if session.state.players.isEmpty {
                        Text("In attesa…")
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                Section {
                    ...
                    .disabled(session.state.players.count < 2)
                } footer: {
                    Text(
                        session.state.players.count < 2
                            ? "Servono almeno 2 giocatori."
                            : "Pronto per iniziare."
                    )
                }
            } else {
                Section("Partite vicine") {
